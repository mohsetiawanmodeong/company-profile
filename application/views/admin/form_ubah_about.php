        ...
        <div class="container-fluid">
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h1 class="h3 mb-2 text-gray-800">Ubah Halaman Tentang Kami</h1>
                </div>
                <div class="card-body">
                    <?php foreach ($about as $abt) : ?>
                        <form method="POST" action="<?=base_url('admin/about/update_about_aksi')?>" Enctype="Multipart/Form-Data">
                            <div class="mb-3">
                                <label for="exampleFormControlInput1">Tagline</label>
                                <input type="hidden" name="id" value="<?php echo $abt->id ?>">
                                <textarea class="form-control" id="tagline" type="text" placeholder="" name="tagline" rows="3"><?php echo $abt->tagline ?></textarea>
                                <?php echo form_error('tagline','<span class="text-small text-danger">','</span>') ?>
                            </div>
                            <div class="mb-3">
                                <label for="exampleFormControlInput1">Deskripsi</label>
                                <textarea class="form-control" id="desc" type="text" placeholder="" name="desc" rows="4"><?php echo $abt->desc ?></textarea>
                                <?php echo form_error('desc','<span class="text-small text-danger">','</span>') ?>
                            </div>
                            <label for="exampleFormControlFile1">Gambar</label>
                            <div class="form-group mb-3">
                            <img class="mb-3" height="250px" src="<?php echo base_url() . 'assets/upload/' . $abt->gambar ?>">
